...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        config.smyrf = True
        config.n_hashes = 2
        config.q_cluster_size = 1
        config.k_cluster_size = 4
        config.attention_probs_dropout_prob = 0.0
        self.max_seq_length = 64

        from smyrf.torch.attn import SmyrfAttention
        self.do_smyrf = config.smyrf

        if self.do_smyrf:
            print('Using smyrf...')
        else:
            print('Not using smyrf...')
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None):
        ...
        # first always with dense attention
        if (Nq != 1) or (not self.do_smyrf):
            # breakpoint()
            w = torch.matmul(q, k)
            ...
            padding_keys = self.max_seq_length - Nk
            k_zeros =  torch.zeros(k.shape[:-1] + (padding_keys,), device=k.device)
            v_zeros =  torch.zeros(v.shape[:-2] + (padding_keys,) + (v.shape[-1],), device=v.device)

            padded_keys = torch.cat((k, k_zeros), -1)
            padded_values = torch.cat((v, v_zeros), -2)
            attention_mask = torch.zeros(padded_keys.shape[:-2] + (padded_keys.shape[-1],), device=k.device)
            attention_mask[:, :, Nk:] = -10000
            breakpoint()
            self.smyrf(q.reshape(-1, Nq, q.shape[-1]), padded_keys.transpose(-2, -1).reshape(-1, self.max_seq_length, k.shape[-2]), padded_values.reshape(-1, self.max_seq_length, v.shape[-1]))
            return q
